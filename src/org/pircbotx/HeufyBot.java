...
public class HeufyBot {
	...
	protected String _realName = "PircBotX";
	...
	protected ListenerManager<? extends HeufyBot> listenerManager = new ThreadedListenerManager();
	/**
		...
		Runnable runnable = new Runnable()
		{
			public void run()
			{
				try
				{
					...
					_name = ((String)settingsMap.get("nickname"));
					_realName = ((String)settingsMap.get("realname"));
					_login = ((String)settingsMap.get("username"));
					_verbose = true;
					_autoNickChange = true;
					...
					switch (authenticationtype)
					{
						...
						case 3:
						connect(serverip, port);
						sendRawLine("AUTH " + _name + " " + password);
						break;
						default:
						connect(serverip, port);
					}
	...
	public synchronized void connect(String hostname, int port, String password, SocketFactory socketFactory) throws IOException, IrcException, NickAlreadyInUseException {
		...
		_outputThread.sendRawLineNow("USER " + getLogin() + " 8 * :" + getRealName());
			...
			if (secondSpace >= 0) {
				...
				if (code.equals("004"))
					...
					if (_autoNickChange) {
						tries++;
						nick = getName() + tries;
						log("Nickname is not available. Changing nick to " + nick, "server");
						_outputThread.sendRawLineNow("NICK " + nick);
					} else {
						_socket.close();
						_inputThread = null;
	...
	public void sendCTCPResponse(User target, String message)
	{
		if (target != null && message != null)
		{
			log("[" + target.getNick() + "]: " + message, "server");
	...
	{
		log(nick + " has been invited to " + channel, "server");
	...
	protected void handleLine(String line)
	{
		...
		if (command.equals("PRIVMSG") && line.indexOf(":\u0001") > 0 && line.endsWith("\u0001")) {
			...
			{
				if (target.equals(this._nick))
			          log(sourceNick + " " + request.substring(7), sourceNick);
			        else
			          log(sourceNick + " " + request.substring(7), target);
		...
		else if (command.equals("PRIVMSG"))
		{
			...
			log(">> " + sourceNick + " (" + sourceLogin + "@" + sourceHostname + ") has joined " + channel.getName(), target);
			...
			if (sourceNick.equals(getNick()))
			{
				...
				log("<< " + sourceNick + " (" + sourceLogin + "@" + sourceHostname + ") has left " + channel.getName() + " (" + message + ")", target);
			...
			if (sourceNick.equals(getNick()))
			{
				...
				setNick(newNick);
				log("Your nick is now " + newNick, "server");
				for(Channel channel2 : user.getChannels())
				{
					log(sourceNick + " is now known as " + newNick, channel2.getName());
				}
			}
		...
		} else if (command.equals("NOTICE"))
		{
			...
			if (channel == null)
		          log("[Notice] --" + sourceNick + "-- " + message, "server");
		        else {
		          log("[Notice] --" + sourceNick + "-- [" + channel.getName() + "] " + message, channel.getName());
		        }
			...
			for (Channel channel2 : snapshot.getChannels())
	        {
			...
			{
				log(recipient.getNick() + " was kicked from " + channel.getName() + " by " + sourceNick + " (" + message + ")", target);
				//Someone else
			...
			if (target.equals(this._nick))
		          log(sourceNick + " sets mode: " + mode, "server");
		        else
		          log(sourceNick + " sets mode: " + mode, target);
			...
			log(sourceNick + " (" + sourceLogin + "@" + sourceHostname + ") invites " + target + " to join " + message, "server");
	 ...
	 */
	public ListenerManager<? extends HeufyBot> getListenerManager() {
	 ...
	 */
	public void setListenerManager(ListenerManager<? extends HeufyBot> listenerManager) {
	...
	public FeatureInterface getFeatureInterface() {
		return featureInterface;
	}

	/**
	 * A listener that waits for the specified event before returning. Used in
	 * {@link HeufyBot#waitFor(java.lang.Class) }
	 */
