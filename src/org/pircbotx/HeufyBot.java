...
public class HeufyBot {
	...
	protected ListenerManager<? extends HeufyBot> listenerManager;
	/**
		...
		try {
			...
			for (InetAddress curAddress : InetAddress.getAllByName(hostname)) {
				...
				try {
					...
					this.log("*** Unable to connect to " + hostname + " using the IP address " + curAddress.getHostAddress() + ", trying to check another address.", "server");
					lastException = t;
				}
			}
			//Make sure were connected
			if (socket == null || (socket != null && !socket.isConnected())) {
				this.log("*** Unable to connect to any working IP address for hostname " + hostname, "server");
			...
			if (webIrcPassword != null) outputThread.sendRawLineNow("WEBIRC " + webIrcPassword + " " + webIrcUsername + " " + webIrcHostname + " " + webIrcAddress.getHostAddress());
			...
			while ((line = breader.readLine()) != null) {
				...
				if (params.size() >= 2) {
					...
					} else if (code.startsWith("5") || code.startsWith("4")) throw new IrcException("Could not log into the IRC server: " + line); else if (code.equals("CAP")) {
						//Handle CAP Code; remove extra from params
						List<String> capParams = Arrays.asList(params.get(2).split(" "));
						if (params.get(1).equals("LS")) for (CapHandler curCapHandler : capHandlers) curCapHandler.handleLS(this, capParams); else if (params.get(1).equals("ACK")) {
							//Server is enabling a capability, store that
							getEnabledCapabilities().addAll(capParams);
							for (CapHandler curCapHandler : capHandlers) curCapHandler.handleACK(this, capParams);
						} else if (params.get(1).equals("NAK")) for (CapHandler curCapHandler : capHandlers) curCapHandler.handleNAK(this, capParams); else 
						//Maybe the CapHandlers know how to use it
						for (CapHandler curCapHandler : capHandlers) curCapHandler.handleUnknown(this, line);
					} else 
				...
				if (capEnabled && !capEndSent) {
					...
					for (CapHandler curCapHandler : capHandlers) if (!curCapHandler.isDone()) {
						allDone = false;
						break;
					}
					if (allDone) {
			...
			if (!(e instanceof IrcException) || !(e instanceof NickAlreadyInUseException)) shutdown(true);
	...
	public void joinChannel(String channel) {
		if (channel == null) throw new IllegalArgumentException("Can\'t join a null channel");
	...
	public void joinChannel(String channel, String key) {
		if (channel == null) throw new IllegalArgumentException("Can\'t join a null channel");
		if (key == null) throw new IllegalArgumentException("Can\'t channel " + channel + " with null key");
	...
	public void partChannel(Channel channel) {
		if (channel == null) throw new IllegalArgumentException("Can\'t part a null channel");
	...
	public void quitServer() {
		if (!isConnected()) throw new RuntimeException("Can\'t quit from server because we are already disconnected!");
		quitServer("");
	}
	/**
	...
	public void sendRawLine(String line) {
		...
		if (isConnected()) outputThread.send(line);
	}
	/**
	...
	public void sendRawLineNow(String line) {
		if (line == null) throw new NullPointerException("Cannot send null messages to server");
		if (isConnected()) outputThread.sendRawLineNow(line);
	}
	...
	public void sendMessage(String target, String message) {
		if (target == null) throw new IllegalArgumentException("Can\'t send message to null target");
		sendRawLineSplit("PRIVMSG " + target + " :", message);
		log("<" + this.nick + "> " + message, target);
	}
	/**
	...
	public void sendMessage(User target, String message) {
		if (target == null) throw new IllegalArgumentException("Can\'t send message to null user");
	...
	public void sendMessage(Channel target, String message) {
		if (target == null) throw new IllegalArgumentException("Can\'t send message to null channel");
	...
	public void sendMessage(Channel chan, User user, String message) {
		if (chan == null) throw new IllegalArgumentException("Can\'t send message to null channel");
	...
	public void sendAction(String target, String action) {
		if (target == null) throw new IllegalArgumentException("Can\'t send action to null target");
		sendCTCPCommand(target, "ACTION " + action);
		log(this.nick + " " + action, target);
	}
	/**
	...
	public void sendNotice(String target, String notice) {
		if (target == null) throw new IllegalArgumentException("Can\'t send notice to null target");
	...
	public void sendNotice(User target, String notice) {
		if (target == null) throw new IllegalArgumentException("Can\'t send notice to null user");
	...
	public void sendNotice(Channel target, String notice) {
		if (target == null) throw new IllegalArgumentException("Can\'t send notice to null channel");
	...
	public void sendCTCPCommand(String target, String command) {
		if (target == null) throw new IllegalArgumentException("Can\'t send CTCP command to null target");
	...
	public void sendCTCPCommand(User target, String command) {
		if (target == null) throw new IllegalArgumentException("Can\'t send CTCP command to null user");
	...
	public void sendCTCPCommand(Channel target, String command) {
		if (target == null) throw new IllegalArgumentException("Can\'t send CTCP command to null channel");
	...
	public void sendCTCPResponse(String target, String message) {
		if (target == null) throw new IllegalArgumentException("Can\'t send CTCP response to null target");
		sendRawLine("NOTICE " + target + " :\u0001" + message + "\u0001");
		log("[" + target + "]: " + message, "server");
	}
	/**
	...
	public void sendCTCPResponse(User target, String message) {
		if (target == null) throw new IllegalArgumentException("Can\'t send CTCP response to null user");
	...
	public void changeNick(String newNick) {
		if (newNick == null) throw new IllegalArgumentException("Can\'t change to null nick");
	...
	public void identify(final String password) {
		if (password == null) throw new IllegalArgumentException("Can\'t identify with null password");
		if (loggedIn) sendRawLine("NICKSERV IDENTIFY " + password); else getListenerManager().addListener(new ListenerAdapter(){
	...
	public void setMode(Channel chan, String mode) {
		if (chan == null) throw new IllegalArgumentException("Can\'t set mode on null channel");
	...
	public void setMode(Channel chan, String mode, Object... args) {
		...
		for (Object curArg : args) argBuilder.append(curArg.toString()).append(" ");
	...
	public void sendInvite(User target, String channel) {
		if (target == null) throw new IllegalArgumentException("Can\'t send invite to null user");
	...
	public void sendInvite(Channel target, Channel channel) {
		if (target == null) throw new IllegalArgumentException("Can\'t send invite to null target channel");
		if (channel == null) throw new IllegalArgumentException("Can\'t send invite to null invite channel");
	...
	public void ban(Channel channel, String hostmask) {
		if (channel == null) throw new IllegalArgumentException("Can\'t set ban on null channel");
	...
	public void unBan(Channel channel, String hostmask) {
		if (channel == null) throw new IllegalArgumentException("Can\'t remove ban on null channel");
	...
	public void setTopic(Channel chan, String topic) {
		if (chan == null) throw new IllegalArgumentException("Can\'t set topic on null channel");
	...
	public void kick(Channel chan, User user, String reason) {
		if (chan == null) throw new IllegalArgumentException("Can\'t kick on null channel");
	...
	protected void handleLine(String line) throws IOException {
		...
		if (senderInfo.startsWith(":")) if (exclamation > 0 && at > 0 && exclamation < at) {
		...
		if (sourceNick.startsWith(":")) sourceNick = sourceNick.substring(1);
		if (target == null) target = !parts.isEmpty() ? parts.get(0) : "";
		...
		{
			if (channel == null) log("[Notice] --" + sourceNick + "-- " + message, "server"); else {
			...
			if (target.equals(this.nick)) log(sourceNick + " sets mode: " + mode, "server"); else log(sourceNick + " sets mode: " + mode, target);
			...
			if (source.getChannels().isEmpty()) userChanInfo.deleteB(source);
		} else 
	...
	protected void processMode(String target, String sourceNick, String sourceLogin, String sourceHostname, String mode) {
		...
		if (channelPrefixes.indexOf(target.charAt(0)) >= 0) {
			...
			for (int i = 0; i < params[0].length(); i++) {
				...
				if (atPos == '+' || atPos == '-') pn = atPos; else if (atPos == 'o') {
	...
	public void setName(String name) {
		if (name == null) throw new IllegalArgumentException("Can\'t set name to null");
		this.name = name;
	}
	/**
	...
	public void setLogin(String login) {
		if (login == null) throw new IllegalArgumentException("Can\'t login to null");
		this.login = login;
	}
	/**
	...
	public void setVersion(String version) {
		if (version == null) throw new IllegalArgumentException("Can\'t version to null");
	...
	public void setFinger(String finger) {
		if (finger == null) throw new IllegalArgumentException("Can\'t finger to null");
		this.finger = finger;
	}
	/**
	...
	public void setEncoding(String charset) throws UnsupportedEncodingException {
		if (charset == null) throw new NullPointerException("Can\'t set charset to null");
	...
	public Set<Channel> getChannels(User user) {
		if (user == null) throw new NullPointerException("Can\'t get a null user");
	...
	public Channel getChannel(String name) {
		if (name == null) throw new NullPointerException("Can\'t get a null channel");
		for (Channel curChan : userChanInfo.getAValues()) if (curChan.getName().equals(name)) return curChan;
	...
	public Set<String> getChannelsNames() {
		return Collections.unmodifiableSet(new HashSet<String>(){
			...
			{
				for (Channel curChan : userChanInfo.getAValues()) add(curChan.getName());
			}
		});
	}
	/**
	...
	public boolean channelExists(String channel) {
		if (channel == null) throw new IllegalArgumentException("Can\'t check for null channel existing");
		for (Channel curChan : userChanInfo.getAValues()) if (curChan.getName().equals(channel)) return true;
		return false;
	}
	/**
	...
	public Set<User> getUsers(Channel chan) {
		if (chan == null) throw new NullPointerException("Can\'t get a null channel");
	...
	public User getUser(String nick) {
		if (nick == null) throw new NullPointerException("Can\'t get a null user");
		if (userNickMap.containsKey(nick)) return userNickMap.get(nick);
	...
	public void setListenerManager(ListenerManager<? extends HeufyBot> listenerManager) {
		if (listenerManager == null) throw new IllegalArgumentException("Can\'t set listener manager to null");
		this.listenerManager = listenerManager;
		//Check if corehooks already exist
		for (Listener curListener : listenerManager.getListeners()) if (curListener instanceof CoreHooks) return;
		listenerManager.addListener(new CoreHooks());
	}
	/**
	...
	public void useShutdownHook(boolean use) {
		if (use && shutdownHook == null) {
			...
			final WeakReference<HeufyBot> thisBotRef = new WeakReference(this);
			Runtime.getRuntime().addShutdownHook(shutdownHook = new Thread(){
				
				
				@Override
				public void run() {
					HeufyBot thisBot = thisBotRef.get();
					if (thisBot != null && thisBot.isConnected() && !thisBot.socket.isClosed()) try {
						thisBot.disconnect();
					} finally {
	...
	public <E extends Event> E waitFor(Class<? extends E> eventClass) throws InterruptedException {
		if (eventClass == null) throw new IllegalArgumentException("Can\'t wait for null event");
	...
	protected class UserChannelMap<C extends Channel, U extends User> extends ManyToManyMap<C, U> {
		...
		public boolean put(C a, U b) {
			//Add to nick map
			if (b != null) userNickMap.put(b.getNick(), b);
		...
		public Set<C> deleteB(U b) {
			...
			synchronized (lockObject) {
				for (Channel curChan : BMap.get(b)) curChan.removeUser(b);
			}
	...
	@java.lang.SuppressWarnings("all")
	public void setWebIrcUsername(final String webIrcUsername) {
		this.webIrcUsername = webIrcUsername;
	}
	
	@java.lang.SuppressWarnings("all")
	public void setWebIrcHostname(final String webIrcHostname) {
	...
	@java.lang.SuppressWarnings("all")
	public void setWebIrcAddress(final InetAddress webIrcAddress) {
	...
	@java.lang.SuppressWarnings("all")
	public void setWebIrcPassword(final String webIrcPassword) {
	...
	@java.lang.SuppressWarnings("all")
	public void setAutoSplitMessage(final boolean autoSplitMessage) {
	...
	public boolean isAutoReconnectChannels() {
		return this.autoReconnectChannels;
	}
	
	@java.lang.SuppressWarnings("all")
	public void setAutoReconnectChannels(final boolean autoReconnectChannels) {
		this.autoReconnectChannels = autoReconnectChannels;
	}
	...
	public void setFeatureInterface(FeatureInterface featureInterface) {
