...
public class IdentServer extends Thread {
	 ...
	 */
	IdentServer(HeufyBot bot, String login) {
		this.bot = bot;
		this.login = login;
		...
		try {
			...
			bot.log("*** Could not start the ident server on port 113.", "server");
			return;
		}

		bot.log("*** Ident server running on port 113 for the next 10 seconds...", "server");
	...
	public void run() {
		try {
			...
			if (line != null) {
				bot.log("*** Ident request received: " + line, "server");
				line = line + " : USERID : UNIX : " + login;
				writer.write(line + "\r\n");
				writer.flush();
				bot.log("*** Ident reply sent: " + line, "server");
				writer.close();
			}
		} catch (Exception e) {
		...
		bot.log("*** The Ident server has been shut down.", "server");
	}
}
