...
public class DccManager implements Closeable {
	...
	protected boolean processRequest(User source, String request) throws DccException, IOException {
		...
		if (type.equals("SEND")) {
			...
			if (transfer == null) throw new DccException("No Dcc File Transfer to resume recieving (filename: " + filename + ", source: " + source + ", port: " + port + ")");
	...
	protected DccFileTransfer removeDccFileTransfer(DccFileTransfer transfer) {
		if (!allFileTransfers.remove(transfer)) throw new RuntimeException("Attempted to forget about DccFileTransfer that doesn\'t exist");
		return transfer;
	}
	/**
	...
	protected DccChat removeDccChat(DccChat chat) {
		if (!allChats.remove(chat)) throw new RuntimeException("Attempted to forget about DccChat that doesn\'t exist");
		return chat;
	}
	/**
	...
	protected ServerSocket createServerSocket() throws IOException, DccException {
		...
		if (ports.isEmpty()) 
		// Use any free port.
		ss = new ServerSocket(0); else {
			for (int currentPort : ports) try {
	...
	public void close() throws IOException {
		...
		for (Closeable curDcc : allDcc) curDcc.close();
	}
